...
from logging import getLogger, Filter, Formatter, Logger
...
def set_logging() -> None:
    ...
    handler: RotatingFileHandler = RotatingFileHandler(Path(lspm_config_dir, 'app.log'),
                                                       maxBytes=10000, backupCount=1)
    formatter: Formatter = Formatter('%(asctime)s - %(name)s - %(levelname)8s - %(message)s')
